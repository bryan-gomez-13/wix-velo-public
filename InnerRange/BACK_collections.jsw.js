...
export const getDropdownsDocuments = webMethod(Permissions.Anyone, async (value) => {
    let results = await wixData.query('Downloads').eq('type', 'Document').hasSome('products', value).limit(100).find();
    let allItems = results.items;
    while (results.hasNext()) {
        results = await results.next();
        allItems = allItems.concat(results.items);
    ...
    const subCategoryOptions = buildSubCategoryOptions(allItems);

    return subCategoryOptions;
})

function buildSubCategoryOptions(items) {
    // Extract all subCategory values into a single array
    const allSubCategories = items.flatMap(item => {
        // Ensure subCategory exists and is an array
        return Array.isArray(item.subCategory) ? item.subCategory : [];
    });

    // Remove duplicates using Set and sort alphabetically
    const uniqueSortedSubCategories = [...new Set(allSubCategories)]
        .sort((a, b) => a.localeCompare(b));

    // Map to the required { label, value } structure
    return uniqueSortedSubCategories.map(subCategory => ({
        label: subCategory,
        value: subCategory
    }));
}
...
export const createSubCategoryForDocuments = webMethod(Permissions.Anyone, async () => {
    let results = await wixData.query('Downloads').eq('type', 'Document').limit(100).find();
    let allItems = results.items;
    while (results.hasNext()) {
        results = await results.next();
        allItems = allItems.concat(results.items);
    ...
    // Add subCategory to each item
    const updatedItems = allItems.map(item => {
        let subCategoryValue = "Manuals & Guides";

        // Validate fileName
        if (item.fileName && typeof item.fileName === "string") {
            if (item.fileName.includes("-")) {
                // Extract text before the first hyphen
                subCategoryValue = item.fileName.split("-")[0].trim();
            }
        }
    ...
    await wixData.bulkUpdate('Downloads', updatedItems, wixDataOptions)
});
