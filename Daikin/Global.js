...
function init() {
  ...
  $w("#captha").onError(() => {
    ...
    $w("#messageTxt")
      .show()
      .then(() => {
        $w("#messageTxt").hide("fade", { delay: 10000 });
      });
  });
...
function activMainMenuBtn(hoveredBtnId) {
    $w('#mainMenuRepeater').forEachItem(($item, itemData, index) => {
        ...
        if (itemData._id === hoveredBtnId) {
            ...
            mainMenuItem.style.color = '#92B079'
        }
    });
}
...
function activFirstLevelRepMenuBtn(hoveredBtnId) {
    $w('#firstLevelRepeater').forEachItem(($item, itemData, index) => {
        ...
        if (itemData._id === hoveredBtnId) {
            ...
            mainMenuItem.style.color = '#f3f8f000' //Transparent
        }
    });
}
...
async function check() {
  if (!$w("#firstName").valid) return "Missing First Name";
  if (!$w("#lastName").valid) return "Missing Last Name";
  if (!$w("#phoneR").valid) return "Missing Phone Number";
  if (!$w("#emailR").valid) return "Missing Email";
  if (!$w("#streetAddress").valid) return "Missing Street Address";
  if (!$w("#suburb").valid) return "Missing Suburb";
  if (!$w("#postcode").valid) return "Missing Postcode";
  if (!$w("#state").valid) return "Missing State";
  if (!$w("#installation").valid) return "Missing Describe your installation";
  if (!$w("#message").valid) return "Missing Message";
  if ($w("#captha").token == undefined)
    return "CAPTCHA authorization failed. Redo the CAPTCHA challenge.";
  else {
    let submitRequestData = { token: $w("#captha").token };
    await processSubmission(submitRequestData).then((response) => {
...
async function saveInfo() {
  ...
  const Comments__c = $w("#message").value;
