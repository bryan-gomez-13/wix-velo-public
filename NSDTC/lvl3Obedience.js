...
function init() {
    ...
    $w("#form1").onSubmitSuccess(async () => {
        try {
            // Get course info
            const lvl3TrainingInfo = await generalQuery_v2("Course", "_id", "9134fb5f-a352-450f-b29b-41bd53ef36b0");
            const itemData = lvl3TrainingInfo?.[0];
            ...
            await Promise.all([
                ...
                appendValuesWrapper(values, "sheetLvL3"),
            ]);
...
async function getLvL3Info() {
    ...
    if (itemData.numberOfPeople <= 0 || itemData.enableForm == false) $w('#boxLvl2').changeState('lvl3Full')
}