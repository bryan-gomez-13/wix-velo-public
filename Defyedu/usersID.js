...
$w.onReady(function () {
    ...
    if (memory.getItem("role")) getInfo(memory.getItem("role"), item)
    else {
        currentMember.getRoles().then((roles) => {
            if (!(roles.length == 0)) {
                memory.setItem("role", roles[0].title)
        ...
        currentMember.getMember()
            .then((member) => {
                wixData.query('users').eq('privateId', member._id).find().then((result) => {
                    ...
                    if (!(item.completeProfile)) $w('#messagePremium').text = "Complete your profile to Initiate a chat session", $w('#messagePremium').show();
                    else initiateChat()
...
function getInfo(role, item) {
    ...
    if (!v6) {
        if (v0 && v1) {
            if (v4 && v5 && item.premium) $w('#chat').show();
            else $w('#chat').hide();
